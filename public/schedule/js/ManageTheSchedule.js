window.RunJS = function RunJS(  parA, parB, parC, parD, parE, parF  ) {	
	...
	///////console.log(JSON.stringify( DEFAULT_EMPLOYEE_ARR ));
	...
	var Positionchoice = React.createClass({ 	
		...
		render: function() { 				
			...
			var lifeSymbol = unCheckedSymbol; var instSymbol = unCheckedSymbol; var headSymbol = unCheckedSymbol; var supeSymbol = unCheckedSymbol;					
			var lifeCLS = "Lifeguard"; var instCLS = "Instructor"; var headCLS = "Headguard"; var supeCLS = "Supervisor";		
			...
			return ( 					
				<div id ="Step3"  >											
					...
					<button  className  = {lifeCLS}  name="Cert" value = "1" id = "Lifeguard"  onClick={this.setposition} >							
						{lifeSymbol} Lifeguard
					</button>
					
					<button  className  = {instCLS} name="Cert" value = "2" id = "Instructor"   onClick={this.setposition} >	
						{instSymbol} Instructor
					</button>
					
					<button  className  = {headCLS}   name="Cert" value = "3" id = "Headguard"    onClick={this.setposition}>		
						{headSymbol} Headguard
					</button>
					
					<button  className  = {supeCLS}  name="Cert" value = "4" id = "Supervisor"  onClick={this.setposition}>	
	...
	var Employeecell = React.createClass({ 	
		setEmployee: function(event) {
			event.preventDefault();
			var EmployeeID = this.props.employee["id"];	
			///////console.log("Employeecell EmployeeID "+EmployeeID);
			if (  EmployeeID == this.props.Selectedid )
			{  this.props.UNsetEmployee(EmployeeID);  } 
			else {  this.props.setEmployee(EmployeeID);  }
		},
		render: function() {
			...
			if( Availability != null ) {  A = Availability[dayofweek];  General = Availability["Generalnote"];  }				
...
<button id = "CreateShiftSubmit" className = {sass}  onClick = { this.TryToCreateEmployee } disabled = {abled} > Create Shift </button>
		...
		render:function() {			
			...
			for( var i = 0; i < 95; i++ ) {			
				if( s < countOfShifts ) {
					...
					///////console.log(currentShiftStart+" "+mysqlTimeArray[i] );
					if( currentShiftStart == mysqlTimeArray[i] ) {
						...
						var ShiftEnd = AMPM12HourArray[ e ]; var ShiftID = shifts[s][ "ShiftID" ];						
						var	e =  mysqlTimeArray.indexOf( currentShiftEnd ) ;
						var hours = (e - i) /4 ;					
						rowblocks.push(
							<div key = {i+170}   className = {sass} > <span className = "textSmall">  <Shiftinfo ShowShiftModal = {this.props.ShowShiftModal} shiftID = {ShiftID} currentShiftEnd = {currentShiftEnd} firstname = {this.props.employee["Firstname"]} lastname = {this.props.employee["Lastname"]} ShiftStart={ShiftStart} ShiftEnd={ShiftEnd}  position={position} />	 { AMPM12HourArray[i]  } </span></div>
						);				
	...
	function weekSwitcher(x) {
		var dayofweek;
		switch(x){case 0: dayofweek = "Monday"; break; case 1: dayofweek = "Tuesday"; break; case 2: dayofweek = "Wednesday"; break; case 3: dayofweek = "Thrusday"; break; case 4: dayofweek = "Friday"; break; case 5: dayofweek = "Saturday"; break; case 6: dayofweek = "Sunday"; break;	 }/* End of switch day */		
		return dayofweek;
	}
	...
	var ManageScheduleeapp = React.createClass({
		...
		loadEmployees: function( s, e, p, dayofweek, yyyymmdd, display, TheScheduleArray ) {									
				...
				var destination = BASE_URL+'reloadEmployeeSelectBox/';					
				var http = new XMLHttpRequest();						 //// at loadEmployees  
		...
		reloadSchedule: function( dayofweek, yyyymmdd, display ) {
				...
				var destination = BASE_URL+'ByGivenDay/';////reloadTheScheduleArraybyGivenDate						
				var http = new XMLHttpRequest();						 //// reloadSchedule 
				http.onreadystatechange = function() {
					if (http.readyState == 4 && http.status == 200) {
						var data = http.responseText;	
				...
				http.open('POST', destination, true);							
				http.setRequestHeader('jsonInput', yyyymmdd);
				http.send();								
			
		},
		TryToCreateEmployee: function(  p, s, e, id, d ) {			
			///////console.log("tryToCreateEmployee: "+id );
			var inputarr = {};		
				inputarr['date'] = d;
				inputarr['StartTime'] = mysqlTimeArray[s];
				inputarr['EndTime'] = mysqlTimeArray[e];
				...
				inputarr['ID'] = id;									
			var jsonItem = JSON.stringify( inputarr );
			var destination = BASE_URL+'CreateTheShift/'; //// CreateTheShift		
		...
		deleteShift: function(shiftID) {			
				...
				var jsonItem = JSON.stringify( inputarr );
				var destination = BASE_URL+'deleteShift/';////deleteShift						
				var http = new XMLHttpRequest();						 //// at deleteShift 
				http.onreadystatechange = function() {
					if (http.readyState == 4 && http.status == 200) {
						var data = http.responseText;
						if (data[0] == '<') { 
